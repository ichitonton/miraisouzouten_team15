...
public class Exp_Ranking_UI : MonoBehaviour
{
    // ★内部の TeamData 定義は削除！既存の Exp_TeamData を使う
    [Header("チームデータ（Red, Blue, Yellowの順）")]
    [SerializeField] private List<Exp_TeamData> allTeamsData;

    // ★RankingSlot（1位、2位、3位の表示枠）を使う
    [Header("ランキング表示スロット（上から1位, 2位...）")]
    [SerializeField] private List<Exp_RankingSlot> rankingSlots;
    ...
    private List<GoalScore_Network> _goalScripts = new List<GoalScore_Network>();

    // 親（UICanvasController）からゴールを受け取る関数
    public void SetupGoals(GoalScore_Network red, GoalScore_Network blue, GoalScore_Network White)
    {
        _goalScripts.Clear();
    ...
    public void UpdateRanking()
    {
        ...
        List<Exp_TeamData> sortedTeams = allTeamsData
    ...
    private void AutoSetupSlots()
    {
        ...
        foreach (Transform child in rankingParentRoot)
        {
            ...
            Exp_RankingSlot slot = child.GetComponent<Exp_RankingSlot>();

            // もしついてなければ、その子供（孫）も探してみる
            if (slot == null)
            {
                slot = child.GetComponentInChildren<Exp_RankingSlot>();
            }
